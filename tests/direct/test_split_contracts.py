...
def test_core_is_unconfigured_until_one_time_binding(direct_vm, direct_deploy, direct_bob):
    ...
    core.bind_evaluator(evaluator_address, "tendercouncil.evaluator.v1", code_hash)
    ...
    assert '"evaluator_code_hash":"sha256:' + "b" * 64 in binding
...
def test_evaluator_rejects_non_core_job_sender(direct_vm, direct_deploy):
    ...
    evaluator = direct_deploy(EVALUATOR, core_address, "tendercouncil.evaluator.v1")
    assert str(evaluator.get_core_address()).lower() == core_address
    assert evaluator.get_evaluator_version() == "tendercouncil.evaluator.v1"
