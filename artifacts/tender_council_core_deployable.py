...
ALLOWED_CHALLENGES=(CHALLENGE_MANDATORY_REQUIREMENT,CHALLENGE_EVIDENCE_OVERLOOKED,CHALLENGE_RUBRIC,CHALLENGE_INTEGRITY)
...
class TenderCouncilCore(gl.Contract):
    ...
    def get_evaluation_context(self,tender_id:str)->str:
        tender=self._tender(tender_id)
        return _canonical({'core_schema_version':CORE_SCHEMA_VERSION,'tender_id':tender_id,'status':tender.status,'evaluation_nonce':int(tender.evaluation_nonce),'snapshot_digest':tender.closed_snapshot_digest})
    @gl.public.view
    ...
    def start_evaluation(self,tender_id:str):
        ...
        self.tenders[tender_id]=tender
        EvaluatorInterface(self.evaluator_address).emit(on='finalized').start_evaluation_job(tender_id,tender.evaluation_nonce,tender.closed_snapshot_digest)
    @gl.public.write
    ...
    def advance_after_response(self,tender_id:str):
        ...
        if admitted==0:
            ...
            self.tenders[tender_id]=tender
            EvaluatorInterface(self.evaluator_address).emit(on='finalized').start_review_job(tender_id,tender.evaluation_nonce,tender.review_nonce,tender.closed_snapshot_digest,tender.evaluation_result_digest,tender.challenge_set_digest)
            return
        self.tenders[tender_id]=tender
    @gl.public.write
    def receive_review_result(self,tender_id:str,evaluation_nonce:u64,review_nonce:u64,snapshot_digest:str,original_result_digest:str,challenge_set_digest:str,decision:str,winner_bid_id:str,result_digest:str):
        ...
        if snapshot_digest!=tender.closed_snapshot_digest or original_result_digest!=tender.evaluation_result_digest or challenge_set_digest!=tender.challenge_set_digest:
